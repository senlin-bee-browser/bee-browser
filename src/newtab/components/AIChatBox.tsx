...
export default function AIChatBox({ currentTabs = [], onAnalyzeTabs }: AIChatBoxProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      content: '您好！我是 Bee Browser 的 AI 助手。我可以帮您分析和管理标签页，或者回答任何相关问题。',
      role: 'assistant',
      timestamp: new Date()
    }
  ])
  ...
  // 添加消息
  const addMessage = useCallback((content: string, role: 'user' | 'assistant', status?: 'loading' | 'complete' | 'error') => {
  ...
  // 更新消息内容（用于流式更新）
  const updateMessage = useCallback((messageId: string, content: string, status?: 'loading' | 'complete' | 'error') => {
    setMessages(prev => 
      prev.map(msg => 
        msg.id === messageId 
          ? { 
  ...
  const handleUserInput = useCallback(async (userInput: string) => {
    ...
    // 为助手创建一个占位消息，用于流式更新
    const assistantMessage = addMessage('正在思考中...', 'assistant', 'loading')

    try {
      // 检查是否是标签页分析相关的请求
      const isTabAnalysisRequest = userInput.toLowerCase().includes('分析') || 
      ...
      if (isTabAnalysisRequest && currentTabs.length > 0) {
        ...
        await CozeAPI.sendMessageStream(
          `请分析以下 ${currentTabs.length} 个网页，总结这些网页的内容：${JSON.stringify(currentTabs.map(tab => ({
            title: tab.title || '无标题',
            url: tab.url || '',
          ...
          })), null, 2)}`,
          {
            ...
            onError: (error) => {
              ...
              updateMessage(assistantMessage.id, '抱歉，分析失败了。', 'error')
              throw error
            }
          },
          {
        ...
        await CozeAPI.sendMessageStream(
          userInput,
          {
            ...
            onError: (error) => {
              console.error('流式输出错误:', error)
              updateMessage(assistantMessage.id, '抱歉，回复失败了。', 'error')
              throw error
            }
          },
          {
      ...
      // 降级到本地预设回复
      const fallbackResponse = await generateFallbackResponse(userInput, currentTabs)
  ...
  const handleAnalyzeTabs = useCallback(async () => {
    ...
    try {
      ...
      // 为助手创建一个占位消息，用于流式更新
      const assistantMessage = addMessage('正在分析标签页...', 'assistant', 'loading')
      ...
      if (apiConnected !== false) {
        try {
          ...
          await CozeAPI.sendMessageStream(
            `请分析以下 ${currentTabs.length} 个网页，总结这些网页的内容：${JSON.stringify(tabsData, null, 2)}`,
            {
              onProgress: (content) => {
                // 实时更新助手消息内容
                updateMessage(assistantMessage.id, content, 'loading')
      ...
      const resultMessage = `分析完成！我已经为您的 ${result.totalTabsAnalyzed} 个标签页创建了 ${result.createdGroups.length} 个分组：\n\n${result.createdGroups.map(group => `• ${group.name} (${group.tabs.length} 个标签页)`).join('\n')}`
      ...
      setMessages(prev => {
        const lastEmptyAssistantIndex = prev.map((msg, index) => 
      ...
      if (!messageUpdated) {
        addMessage('抱歉，标签页分析失败了。请检查网络连接后重试。', 'assistant')
      }
  ...
  const bubbleItems = messages.map(msg => ({
    ...
    typing: msg.typing ? { step: 5, interval: 20 } : false,
    classNames: {
      content: msg.status === 'loading' ? 'animate-pulse bg-gradient-to-r from-blue-50 to-blue-100' : ''
    }
  }))
      ...
      title={
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <WifiOutlined 
              className={`text-sm ${
                apiConnected === true ? 'text-green-500' : 
                apiConnected === false ? 'text-red-500' : 'text-gray-400'
              }`}
              title={
                apiConnected === true ? 'Coze API 已连接' : 
                apiConnected === false ? 'API 连接失败，使用本地模式' : '检查连接中...'
              }
            />
            {apiConnected === false && (
              <Button 
    ...
    >
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="p-4 border-t">
          <Sender
            placeholder={apiConnected === false ? "AI 服务不可用，使用本地模式..." : "输入您的问题..."}
...
// 降级回复函数（当 API 不可用时使用）
async function generateFallbackResponse(userInput: string, tabs: chrome.tabs.Tab[]): Promise<string> {
  const responses = {
  ...
  // 检查是否是标签页分析相关的请求
  const isTabAnalysisRequest = input.includes('分析') || 
                              input.includes('标签') ||
                              input.includes('分组') ||
                              input.includes('整理')
